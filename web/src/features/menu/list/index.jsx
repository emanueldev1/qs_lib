...
const useStyles = createStyles((theme, params) => ({
  ...
  container: {
    ...
    pointerEvents: "none",
    marginTop:
      params.position === "top-left" || params.position === "top-right" ? 5 : 0,
    marginLeft:
      params.position === "top-left" || params.position === "bottom-left"
        ? 5
        : 0,
    marginRight:
      params.position === "top-right" || params.position === "bottom-right"
        ? 5
        : 0,
    marginBottom:
      params.position === "bottom-left" || params.position === "bottom-right"
        ? 5
        : 0,
    right:
      params.position === "top-right" || params.position === "bottom-right"
...
const ListMenu = () => {
  ...
  const handleCloseMenu = (ignoreFetch, keyPressed, forceClose) => {
    if (menu.canClose === false && !forceClose) return
    setVisible(false)
    if (!ignoreFetch) fetchNui("handleCloseMenu", keyPressed)
  }
  ...
  const moveMenu = e => {
    ...
    switch (e.code) {
      ...
      case "Enter":
        ...
        fetchNui("handleConfirmSelected", [selected, indexStates[selected]]).catch()
        if (
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      fetchNui("handleChangeChecked", [selected, checkedStates[selected]]).catch()
    }, 100)
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      fetchNui("handleChangeIndex", [selected, indexStates[selected]]).catch()
    }, 100)
    return () => clearTimeout(timer)
  }, [indexStates])
  ...
  useEffect(() => {
    ...
    const keyHandler = e => {
      if (["Escape", "Backspace"].includes(e.code)) handleCloseMenu(false, e.code)
    }
  ...
  const isValuesObject = useCallback(
    values => {
      return (
        ...
        typeof values[indexStates[selected]] === "object"
      )
    },
    [indexStates, selected]
  )

  useNuiEvent("handleCloseMenu", () => handleCloseMenu(true, undefined, true))

  useNuiEvent("setMenu", data => {
    firstRenderRef.current = true
    if (!data.startItemIndex || data.startItemIndex < 0) data.startItemIndex = 0
    else if (data.startItemIndex >= data.items.length)
      data.startItemIndex = data.items.length - 1
