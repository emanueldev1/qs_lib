...
--- @module inventory
--- @
RegisterNetEvent(GetCurrentResourceName() .. ':bridge:ox_inventoy:openStash', function(stashId, data)
   if not data then data = { maxweight = 150000, slots = 50 } end
   exports.ox_inventory:openInventory('stash', lib.table.concat({ id = stashId }, data or {}))
end)
...
function Inventory.isOpen()
   -- OX lib uses state bags to track the inventory state
...
function Inventory.registerStash(stashId, data)
   ...
   TriggerServerEvent(GetCurrentResourceName() .. ':bridge:ox_inventoy:registerStash', stashId, data)
end
