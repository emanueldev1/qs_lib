...
--- Specifically designed for integration with the ak47-inventory system.
--- @module inventory
...
function Inventory.getWeaponList()
   lib.print.debug("ak47_inventory does not support GetWeaponList functions yet, returning the item list.")
   return exports['ak47_inventory']:Items()
end
...
function Inventory.get(id)
   return exports['ak47_inventory']:GetInventory(id)
end
...
function Inventory.addItem(source, itemName, itemCount, itemSlot, itemMetadata)
   return exports['ak47_inventory']:AddItem(source, itemName, itemCount, itemSlot, itemMetadata)
end
...
function Inventory.removeItem(source, itemName, itemCount, itemSlot, itemMetadata)
   return exports['ak47_inventory']:RemoveItem(source, itemName, itemCount, itemSlot)
end
...
function Inventory.canCarryItem(source, itemName, itemCount)
   return exports['ak47_inventory']:CanAddItem(source, itemName, itemCount)
end

--- Retrieves the total amount of a specific item in a player's inventory.
--- @param source number|string The player source ID.
--- @param itemName string The name of the item.
--- @return number The total amount of the item.
function Inventory.getItemTotalAmmount(source, itemName)
   return exports['ak47_inventory']:GetAmount(source, itemName)
end
...
function Inventory.getItemLabel(item)
   return exports['ak47_inventory']:Items(item)?.label or ""
end
...
function Inventory.hasItem(invId, item, count, md, slot)
   return Inventory.getItemTotalAmmount(invId, item) >= (count or 1)
end
...
function Inventory.setItemMetadata(source, itemSlot, itemMetadata)
   exports['ak47_inventory']:SetItemInfo(source, itemSlot, itemMetadata)
end
...
function Inventory.registerStash(source, stashId, data)
   ...
   exports['ak47_inventory']:OpenInventory(source, stashId)
   return true
end
...
function Inventory.addItemToStash(stashId, itemName, itemAmount, itemSlot, itemMetadata, stashSlots, stashMaxWeight)
   return Inventory.addItem(stashId, itemName, itemAmount, itemSlot, itemMetadata)
end
...
function Inventory.removeItemFromStash(stashId, itemName, itemAmount, itemSlot, stashSlots, stashMaxWeight)
   return Inventory.removeItem(stashId, itemName, itemAmount, itemSlot)
end
...
function Inventory.getStashItems(stashId)
   return exports['ak47_inventory']:GetInventory(stashId)
end
...
function Inventory.getItemBySlot(source, slot)
   return exports['ak47_inventory']:GetSlot(source, slot)
end
