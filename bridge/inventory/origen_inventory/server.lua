...
--- Specifically designed for integration with the origen_inventory system.
--- @module inventory
...
function Inventory.getWeaponList()
   lib.print.debug("origen_inventory does not support GetWeaponList functions yet, returning the item list.")
   return exports.origen_inventory:Items()
end
...
function Inventory.addItem(source, itemName, itemCount, itemSlot, itemMetadata)
   return exports.origen_inventory:addItem(source, itemName, itemCount, itemMetadata, itemSlot)
end
...
function Inventory.removeItem(source, itemName, itemCount, itemSlot, itemMetadata)
   return exports.origen_inventory:removeItem(source, itemName, itemCount, itemMetadata, itemSlot)
end
...
function Inventory.canCarryItem(source, itemName, itemCount)
   return exports.origen_inventory:canCarryItem(source, itemName, itemCount)
end
...
function Inventory.getItemTotalAmmount(source, itemName)
   return exports.origen_inventory:getItemCount(source, itemName)
end
...
function Inventory.getItemLabel(item)
   ...
   return itemData and itemData.label or itemData.name or item
end
...
function Inventory.setItemMetadata(source, itemSlot, itemMetadata)
   exports.origen_inventory:setMetadata(source, itemSlot, itemMetadata)
end
...
function Inventory.registerStash(source, stashId, data)
   ...
   if source then
      TriggerClientEvent(GetCurrentResourceName() .. 'bridge:origen_inventory:openStash', source, stashId, {
         label = data.label,
...
RegisterNetEvent(GetCurrentResourceName() .. 'bridge:origen_inventory:registerStash', function(stashId, data)
   Inventory.registerStash(0, stashId, data)
end)
