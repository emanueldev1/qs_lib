...
-- playAnim.lua: Plays a specified animation on a ped in FiveM.
-- This module loads an animation dictionary, applies the animation to the given ped with customizable parameters,
-- and ensures proper cleanup of resources after execution.
...
-- Loads an animation dictionary and ensures it is ready for use.
---@param dict string
local function loadAnimDict(dict)
   lib.requestAnimDict(dict)
end

-- Cleans up an animation dictionary after use.
---@param dict string
local function unloadAnimDict(dict)
   RemoveAnimDict(dict)
end

-- Normalizes animation parameters, applying default values where needed.
---@param params table
---@return table
local function normalizeParams(params)
   return {
      blendIn = params.blendInSpeed or 7.5,
      blendOut = params.blendOutSpeed or -7.5,
      duration = params.duration or -1,
      animFlags = params.animFlags or 0,
      startPhase = params.startPhase or 0.0,
      phaseControlled = params.phaseControlled or false,
      controlFlags = params.controlFlags or 0,
      overrideCloneUpdate = params.overrideCloneUpdate or false
   }
end
...
---@param blendInSpeed? number Defaults to 7.5
---@param blendOutSpeed? number Defaults to -7.5
...
function lib.playAnim(ped, animDictionary, animationName, blendInSpeed, blendOutSpeed, duration, animFlags, startPhase, phaseControlled, controlFlags, overrideCloneUpdate)
   ...
   local params = normalizeParams({
      ...
      overrideCloneUpdate = overrideCloneUpdate
   })
   ...
   TaskPlayAnim(
      ...
      params.animFlags,
      params.startPhase,
      params.phaseControlled,
      params.controlFlags,
      params.overrideCloneUpdate
   )

   -- Clean up animation dictionary
   unloadAnimDict(animDictionary)
end
